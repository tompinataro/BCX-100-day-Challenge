...
cron.schedule('0 * * * *', async () => {
  ...
  } catch (err) {console.log('error running reminders!', err)};

  try {
    const endOfDayQuery = `SELECT u.id, u.email, u.first_name, u."dayEndTime", u.notify, up.step
    FROM "user" u
    ...
    SELECT 
          ...
          FROM "dailyhabits" dh 
          WHERE dh.user_id = 1 AND dh.date = CURRENT_DATE 
      ) < up.step;
    `
